...
tensor_field_raise_or_lower_idx_in_place_cu_kernel(const struct gkyl_tensor_field *met, int raised_idx, 
  ...
  struct gkyl_tensor_field *ten)
{
  ...
  tensor_field_raise_or_lower_idx_in_place_cu_kernel<<<dimGrid, dimBlock>>>(met->on_dev, raised_idx, ten->on_dev);
}
