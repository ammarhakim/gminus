...
gkyl_vlasov_lte_moments_inew(const struct gkyl_vlasov_lte_moments_inp *inp)
{
  ...
  if (up->model_id == GKYL_MODEL_SR) {
    ...
    struct gkyl_mom_canonical_pb_auxfields can_pb_inp = {.hamil = inp->hamil, .energy = inp->energy}; 
    ...
    up->M1i_calc = gkyl_dg_updater_moment_new(inp->phase_grid, inp->conf_basis,
      inp->phase_basis, inp->conf_range, inp->vel_range, inp->phase_range, up->model_id, &can_pb_inp, "M1i_from_H", false, inp->use_gpu);
    ...
    else if (up->model_id == GKYL_MODEL_CANONICAL_PB_GR) {
   ...
   // spatial components of the four-velocity squared, u_i^2 = (GammaV*V_drift)^2
    ...
    up->gr_vars = gkyl_dg_calc_gr_vars_new(inp->phase_grid, inp->vel_grid, 
      inp->conf_basis, inp->vel_basis, inp->phase_basis, inp->conf_range, inp->vel_range, inp->use_gpu);
  ...
  const struct gkyl_array *fin, struct gkyl_array *moms_out)
{
  ...
  if (lte_moms->model_id == GKYL_MODEL_SR) {
    ...
    // M1i computes contravariant u^i, convert here to covariant u_i
...
gkyl_vlasov_lte_moments_release(gkyl_vlasov_lte_moments *lte_moms)
{
  ...
  if (lte_moms->model_id == GKYL_MODEL_SR) {
    ...
    gkyl_dg_calc_gr_vars_release(lte_moms->gr_vars);
    gkyl_array_release(lte_moms->V_drift_sq);
    gkyl_array_release(lte_moms->GammaV);
    gkyl_array_release(lte_moms->GammaV_sq);
  }
  ...
  if (lte_moms->model_id != GKYL_MODEL_SR && lte_moms->model_id != GKYL_MODEL_CANONICAL_PB_GR) {
