...
GKYL_CU_DH void canonical_pb_vars_util_2x3v_tensor_p1(const double *h_ij_inv, const double *v_i, const double *u_j, double* GKYL_RESTRICT v_dot_u) 
{ 
  ...
  const double *Vy = &v_i[4]; 
  const double *Vz = &v_i[8]; 

  const double *Ux = &u_j[0]; 
  const double *Uy = &u_j[4]; 
  const double *Uz = &u_j[8]; 

  const double *Hxx = &h_ij_inv[0]; 
  const double *Hxy = &h_ij_inv[4]; 
  const double *Hxz = &h_ij_inv[8]; 
  const double *Hyy = &h_ij_inv[12]; 
  const double *Hyz = &h_ij_inv[16]; 
  const double *Hzz = &h_ij_inv[20]; 

  // h^{ij}*v_i*u_j 
  double Hxx_Vx[4] = {0.0}; 
  double Hxx_Vx_Ux[4] = {0.0}; 
  binop_mul_2d_ser_p1(Hxx, Vx, Hxx_Vx); 
  binop_mul_2d_ser_p1(Hxx_Vx, Ux, Hxx_Vx_Ux); 
 
  double Hxy_Vx[4] = {0.0}; 
  double Hxy_Vx_Uy[4] = {0.0}; 
  binop_mul_2d_ser_p1(Hxy, Vx, Hxy_Vx); 
  binop_mul_2d_ser_p1(Hxy_Vx, Uy, Hxy_Vx_Uy); 
 
  double Hxy_Ux[4] = {0.0}; 
  double Hxy_Ux_Vy[4] = {0.0}; 
  binop_mul_2d_ser_p1(Hxy, Ux, Hxy_Ux); 
  binop_mul_2d_ser_p1(Hxy_Ux, Vy, Hxy_Ux_Vy); 
 
  double Hyy_Vy[4] = {0.0}; 
  double Hyy_Vy_Uy[4] = {0.0}; 
  binop_mul_2d_ser_p1(Hyy, Vy, Hyy_Vy); 
  binop_mul_2d_ser_p1(Hyy_Vy, Uy, Hyy_Vy_Uy); 
 
  double Hxz_Vx[4] = {0.0}; 
  double Hxz_Vx_Uz[4] = {0.0}; 
  binop_mul_2d_ser_p1(Hxz, Vx, Hxz_Vx); 
  binop_mul_2d_ser_p1(Hxz_Vx, Uz, Hxz_Vx_Uz); 
 
  double Hxz_Ux[4] = {0.0}; 
  double Hxz_Ux_Vz[4] = {0.0}; 
  binop_mul_2d_ser_p1(Hxz, Ux, Hxz_Ux); 
  binop_mul_2d_ser_p1(Hxz_Ux, Vz, Hxz_Ux_Vz); 
 
  double Hyz_Vy[4] = {0.0}; 
  double Hyz_Vy_Uz[4] = {0.0}; 
  binop_mul_2d_ser_p1(Hyz, Vy, Hyz_Vy); 
  binop_mul_2d_ser_p1(Hyz_Vy, Uz, Hyz_Vy_Uz); 
 
  double Hyz_Uy[4] = {0.0}; 
  double Hyz_Uy_Vz[4] = {0.0}; 
  binop_mul_2d_ser_p1(Hyz, Uy, Hyz_Uy); 
  binop_mul_2d_ser_p1(Hyz_Uy, Vz, Hyz_Uy_Vz); 
 
  double Hzz_Vz[4] = {0.0}; 
  double Hzz_Vz_Uz[4] = {0.0}; 
  binop_mul_2d_ser_p1(Hzz, Vz, Hzz_Vz); 
  binop_mul_2d_ser_p1(Hzz_Vz, Uz, Hzz_Vz_Uz); 
