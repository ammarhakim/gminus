...
GKYL_CU_DH void gr_vars_n_copy_2x3v_ser_p1(int count, const double *h_ij, struct gkyl_nmat *x, const double *M0, double* GKYL_RESTRICT n) 
{ 
  ...
  double V_2[4] = {0.0}; 
  V[8] = gkyl_mat_get(&x2,0,0);   V_2[0] = gkyl_mat_get(&x2,0,0); 
  V[9] = gkyl_mat_get(&x2,1,0);   V_2[1] = gkyl_mat_get(&x2,1,0); 
  V[10] = gkyl_mat_get(&x2,2,0);   V_2[2] = gkyl_mat_get(&x2,2,0); 
  V[11] = gkyl_mat_get(&x2,3,0);   V_2[3] = gkyl_mat_get(&x2,3,0); 
 
  double V_sq[4] = {0.0}; 
  canonical_pb_vars_util_2x3v_ser_p1(h_ij, V, V, V_sq); 
  ...
  if (cell_avg) { 
    ...
    hxz_quad = 1.5*Hxz[3]-0.8660254037844386*Hxz[2]-0.8660254037844386*Hxz[1]+0.5*Hxz[0]; 
    hyy_quad = 1.5*Hyy[3]-0.8660254037844386*Hyy[2]-0.8660254037844386*Hyy[1]+0.5*Hyy[0]; 
    hyz_quad = 1.5*Hyz[3]-0.8660254037844386*Hyz[2]-0.8660254037844386*Hyz[1]+0.5*Hyz[0]; 
    hzz_quad = 1.5*Hzz[3]-0.8660254037844386*Hzz[2]-0.8660254037844386*Hzz[1]+0.5*Hzz[0]; 
